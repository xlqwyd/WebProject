...
<body>
	<%
		...
		if(session.getAttribute("id") != null){
			...
			mtype = (String)session.getAttribute("mtype"); //관리자 인지 확인하는 변수
		}
		
		
		String s_type = request.getParameter("s_type");  //검색 양식
		String s_value = request.getParameter("s_value"); //검색한 것
			  ...
			  <%
    			...
    			<button type="button" class="btn btn-primary active dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
    				...
    				<ul class="dropdown-menu" role="menu">
      				...
      				<li class=active><a href="/heatmap/result">전체 측정 기록</a></li>      <!-- 관리자가 전체 측정보는곳 -->
	...
	<div class="container">
			...
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				} else list = formdao3.selectAll();  //검색한게 없을시 전체 보여주기
	...
	<div>
		<form method="post" action="result2">
			<select name="s_type">
			<option value="1">측정일시</option>
			<option value="2">측정위치</option>
			<option value="3">배정번호</option>
			<option value="4">등록자아이디</option>
			</select>
	...
	<!--  
	<div>
	<button id="button1" onclick="button1_click();">엑셀다운</button>
	<script>
	function button1_click() {
		...
		$.ajax({
			...
			data : { s_type : $("select [name = s_type]").val(),
					s_value : $("input [name = s_value]").val()},

			success : function(data){ alert('비밀번호 불일치2.'); }
